    ...
    <div class="betting-slip-content">
      ...
      <div class="stake-input">
        <ion-item>
          ...
          <ion-input type="number" :value="stakeAmount" min="1" @input="stakeAmount = $event.target.value"
            placeholder="Enter your stake"></ion-input>
        </ion-item>
      </div>
      ...
      <ion-button @click="placeBet">Place Bet</ion-button>
    </div>
  </div>
</template>
...
import { apiData, calculateTotalOdds, filterSortedGames, placeBetAndUpdateBalance, selectedTeams, user } from '../dataCollection';
...
const placeBet = async () => {
  const betData = {
    stake: Number(stakeAmount.value),
  ...
  try {
    const docRef = await addDoc(betCollectionRef, betData);
    console.log('Document written with ID: ', docRef.id); // Firestore generates a unique ID for your bet
    localStorage.clear();
    placeBetAndUpdateBalance(String(user.value?.uid), stakeAmount.value)
    selectedTeams.value = {} as SelectedTeams;
    showBettingSlip.value = false;
    stakeAmount.value = 0;
  } catch (e) {
...
.game-box {
  ...
  margin-bottom: 20px;
  /* Increased margin for more space between bets */
  border-radius: 10px;
